...
def main():
    ...
    # These values are taken directly from the frozen checkpoint.
    seed = 42
    train_fraction = 0.70
    val_fraction = 0.15
    test_fraction = 0.15
    ...
    if len(dataset) != 15_579:
        raise ValueError(
            f"Expected the original 15,579 proteins, got {len(dataset):,}. "
            "DO NOT reconstruct the split from a different dataset."
        )
    ...
    save_dataset_splits(
        dataset=dataset,
        splits=splits,
        output_path="data/splits/protein_splits.json",
        seed=seed,
    )
